...
def test():
    ...
    while True:
        ...
        for i in range(len(text_index)):
            temp = []
            for j in range(len(text_index[i])):
                temp.append('O')
            test_tag.append(temp)
        # test data
        test_dataset = MyDataset(text_index, test_tag, char_to_index, tag_to_id, id_to_radical)
        ...
        if BI_LSTM_CRF:
            ...
            with torch.no_grad():
                ...
                for test_batch_char_index, test_batch_tag_index, batch_len, test_batch_onerad_index in test_dataloader:
                    ...
                    # score
                    score = model.prediction.cpu().numpy().tolist()
                    append_all_pre_test.append(score)

                    # get characters
                    char = test_batch_char_index[:, :-1].detach().cpu().numpy().reshape(-1).tolist()
                    append_all_char_test.append(char)

                words_append = []
                prediction_append = []

                for i in range(len(append_all_pre_test)):
                    words_append.append([id_to_char[j] for j in append_all_char_test[i]])
                    prediction_append.append([id_to_tag[j] for j in append_all_pre_test[i]])

                print(text_index)
                print(prediction_append)
                # print(ground_truth_append)
...
def final_test_BiLSTM_CRF(test_dataloader):
    ...
    with torch.no_grad():
        ...
        for test_batch_char_index, test_batch_tag_index, batch_len, test_batch_onerad_index in test_dataloader:
            ...
            # pre
            score = pre_tag[:, :-1].detach().cpu().numpy().reshape(-1).tolist()
            all_pre_test.extend(score)
            append_all_pre_test.append(score)
            # gold tag
            golden = test_batch_tag_index[:, :-1].detach().cpu().numpy().reshape(-1).tolist()
            all_tag_test.extend(golden)
            append_all_tag_test.append(golden)
            # get characters
            char = test_batch_char_index[:, :-1].detach().cpu().numpy().reshape(-1).tolist()
            all_char_test.extend(char)
            append_all_char_test.append(char)
        ...
        if SHOW_REPORT:  # true
            ...
            for i in range(len(append_all_pre_test)):
                words_append.append([id_to_char[j] for j in append_all_char_test[i]])
                prediction_append.append([id_to_tag[j] for j in append_all_pre_test[i]])
                ground_truth_append.append([id_to_tag[j] for j in append_all_tag_test[i]])

            with open('Report/Report_word_predict_truth.txt', 'w', encoding='utf-8') as f:
                for i in range(len(words_append)):
                    for j in range(len(words_append[i])):
                        f.write(words_append[i][j] + '\t' + prediction_append[i][j] + '\t' + ground_truth_append[i][j] + '\n')
                    f.write('\t'+'\n')
...
def final_test_BiLSTM(test_dataloader):
    ...
    with torch.no_grad():
        ...
        for test_batch_char_index, test_batch_tag_index, batch_len, test_batch_onerad_index in test_dataloader:
            ...
            golden = test_batch_tag_index.cpu().numpy().reshape(-1).tolist()
            all_tag_test.extend(golden)
            append_all_tag_test.append(golden)
            # get characters
            char = test_batch_char_index[:, :-1].detach().cpu().numpy().reshape(-1).tolist()
...
if __name__ == "__main__":
    ...
    ## reduce 100-300
    hidden_num = 450  # one direction ; bi-drectional = 2 * hidden
    bi = True
    # both direction
    lr = 0.001
